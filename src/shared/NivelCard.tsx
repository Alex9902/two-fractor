...
const DIFICULTAD_LABEL: Record<LevelManifest["dificultad"], string> = {
  facil: "Fácil",
  medio: "Medio",
  dificil: "Difícil",
  ...
  return (
    <div
      className={`h-full border-4 border-black p-6 sm:p-8 flex flex-col justify-between transition-all relative ${
        estaDesbloqueado
          ? `${manifest.color} cursor-pointer shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] sm:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]`
          : "bg-[#E5E5E0] text-black/40 border-black/40 shadow-none cursor-not-allowed"
      }`}
      onClick={() => estaDesbloqueado && onSelect(manifest.id)}
    >
      <div className="flex justify-between items-center mb-4 min-h-[28px]">
        {/* Dificultad */}
        <span className={`text-xs font-black uppercase border-2 px-2 py-0.5 ${estaDesbloqueado ? "border-black bg-white/70 text-black" : "border-black/20 text-black/30"}`}>
          {DIFICULTAD_LABEL[manifest.dificultad]}
        </span>
      ...
      <div>
        <h2 className="text-lg md:text-base lg:text-xl font-black uppercase mb-2 leading-tight">
          {manifest.titulo}
        </h2>
